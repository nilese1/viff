...
async def test_monitored_url_crud(db_session: AsyncSession) -> None:
    ...
    urls = await url_repo.list_all(db_session)
    assert [url.id for url in urls] == [monitored_url.id]

    inactive_urls = await url_repo.list_all(db_session, active_only=True)
    assert [url.id for url in inactive_urls] == [monitored_url.id]
    ...
    active_urls = await url_repo.list_all(db_session, active_only=True)
    assert active_urls == []
